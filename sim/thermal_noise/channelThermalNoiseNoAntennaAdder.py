...
class channelThermalNoiseNoAntennaAdder:
    """
    ...
    def run(
        ...
        for i_pixel in range(healpy.pixelfunc.nside2npix(self.__n_side)):
            ...
            for channel in station.iter_channels():
                ...
                # take antenna vel to be of ideal antenna for testing
                # inspired by https://www.waves.utoronto.ca/prof/svhum/ece422/notes/09a-receiving.pdf
                # ignoring all constants to set ideal vpol oriented along z axis (see p 5)
                antenna_response = {'theta' : 1 + 1j, 'phi' : 0} if channel.get_id() in [0, 1, 2, 3] else {'theta' : 0, 'phi' : 1 + 1j}
