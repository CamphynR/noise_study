...
class channelGalacticSunNoiseAdder:
    """
    ...
    def get_noise_temperatures(self):
        return self.__noise_temperatures
...
def read_quiet_sun_json(json_path):
    ...
    for key_value_pair in content:
        freq = float(key_value_pair["x"]) * units.MHz
        temp = float(key_value_pair["y"]) * units.kelvin
        sun_dic["frequency"].append(freq)
        sun_dic["temperature"].append(temp)
