...
if __name__ == "__main__":
    ...
    # S parameters
    det_sparam = Detector(select_stations=station_ids)
    det_sparam.update(dt.datetime(season, 6, 1))
    ...
    for station_index, station_id in enumerate(station_ids):
        ...
        for channel_set_index, (channel_set_name, channel_ids) in enumerate(channel_id_sets.items()):
            ...
            for channel_index, channel_id in enumerate(channel_ids):
                ...
                resp = cal_gain[channel_id] * calibration_response_helper.get_response(channel_id)["gain"](freqs)
                resp = interpolate.interp1d(freqs, resp, bounds_error=False, fill_value=0+0j)
