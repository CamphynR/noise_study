...
if __name__ == "__main__":
    ...
    season = 2023
    digitizer = "digitizer_v2" if season < 2024 else "digitizer_v3"
...
#        trace_cal = np.sum(fft.freq2time(calibration_response * spec_per_batch, sampling_rate), axis=0)
        ...
        spec_cal = np.sum(calibration_response * spec_per_batch, axis=0)
        trace_cal = fft.freq2time(spec_cal, sampling_rate)
...
#        ax.plot(times[::d_gain], vrms_per_run[channel_id]/units.mV, marker="v", label="Calibrated per run", lw=1., color="pink")
        ax.hlines(np.sqrt(np.mean((vrms[channel_id]/units.mV)**2)), xmin=times[0], xmax=times[-1], label = "power vrms", colors="green")
...
#-------------------- PLOTS FOR PAPER ------------------
    ...
    fig.tight_layout()
    fig.savefig(f"figures/paper/compare_rms_system_responses_season{season}_st{station_id}.png", dpi=300)
