...
# -----BANDPASS-------
    ...
    filt = bandpass.get_filter(frequencies, station_id=0, channel_id=0, det=0,
                               passband=passband,
...
#            )
    ...
    for event_batch in thermal_noise_trace_paths[:1]:
        ...
        vrms_s_param += list(vrms_s_tmp)

        vrms_cal_tmp = np.sqrt(np.mean(trace_cal**2, axis=-1))
        vrms_calibration += list(vrms_cal_tmp)
...
#------------DATA-------------
    ...
    times_date = [Time(t, format="unix").strftime("%b-%d") for t in times]
...
#----------------PLOTS------------------
    ...
    pdf = PdfPages(f"figures/paper/compare_rms_system_responses_season{season}_st{station_id}.pdf")
    for channel_id in channel_ids:
        fig, ax = plt.subplots()
        ax.errorbar(times, vrms[channel_id]/units.mV, yerr=var_vrms[channel_id]/units.mV,
                    marker="o", label="Vrms", ls=None, markerfacecolor="none")
#        ax.plot(times[::d_gain], vrms_per_run[channel_id]/units.mV,
#                marker="v", label="Calibrated\nper run", lw=1., color="pink")
        ax.hlines(vrms_calibration[channel_id]/units.mV, xmin=times[0], xmax=times[-1], label = "calibrated\nresponse", colors="red")
        ax.hlines(vrms_s_param[channel_id]/units.mV, xmin=times[0], xmax=times[-1], label = "combined S\nparameter\nresponse", colors="blue")
        ax.set_xlabel("date")
