...
def read_hist_from_data_dir(data_dir, nr_bins, channels):
    hist_output_list = []
    spec_hist = np.zeros((len(channels), nr_bins))
    ...
    for run_file in run_files:
        ...
        hist_output = np.array(hist_output)
        for hist_output_event in hist_output:
            for channel in range(len(channels)):
                spec_hist[channel, hist_output_event[channel, 1]] += 1
...
if __name__ == "__main__":
    ...
    freqs, spec_hists, sampling_rate = read_hist_from_data_dir(args.data_dir, nr_bins,
    ...
    pdf = PdfPages(f"figures/spec_hist/spec_hist_s{station_nr}_f{freq}.pdf")
