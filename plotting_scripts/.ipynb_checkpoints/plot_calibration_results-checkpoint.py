...
def save_calibration_results(result_dic, config, directory, format="csv"):
    station_id = config["station"]
    season = config["season"]
    filename = "absolute_amplitude_calibration" + "_" + f"season{season}" + "_" + f"st{station_id}" + "." + format
    filename = directory + "/" + filename
    with open(filename, "w") as file:
        fieldnames = config["channels_to_include"]
        writer = csv.DictWriter(file, fieldnames=fieldnames)
        writer.writeheader()
        writer.writerow({ch_id : result_dic[ch_id]["gain"][0] for ch_id in config["channels_to_include"]})
...
if __name__ == "__main__":
    ...
    plt.style.use("retro")
    pdf = PdfPages(f"figures/absolute_ampl_calibration/spectra_fit_season{config['season']}_st{station_id}.pdf") 
    ...
    with open(f"absolute_amplitude_fits/absolute_gains_fit_season{config['season']}_s{station_id}.pickle", "wb") as pickle_file:
        pickle.dump(fit_results_dic, pickle_file)

    save_calibration_results(fit_results_dic, config, directory="/user/rcamphyn/noise_study/absolute_amplitude_results")
...
#    fig.suptitle(f"Calibration results", size=62)
    ...
    plt.savefig(f"figures/POS_ICRC/spectra_fit_season{config['season']}_st{station_id}_pos.pdf", bbox_inches="tight") 
