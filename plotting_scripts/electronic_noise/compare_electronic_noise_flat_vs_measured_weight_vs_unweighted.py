...
if __name__ == "__main__":
    ...
    calibration_types = ["default", "no_electronic_weight", "no_measured_electronic_noise", "no_measured_electronic_noise_no_electronic_weight"]
    ...
    # with flat electronic noise (80 K)
    sim_dir_flat_electronic_noise = os.path.join(sim_dir_base, 
                                                 "complete_sim_average_ft_set_v0.2_no_system_response",
                                                 f"digitizer_v{digitizer_version}"
                                                 )
    sim_flat_electronic_noise = load_sim_components(sim_dir_flat_electronic_noise)

    if args.debug:
        print("DONE PARSING SIM")


    # we have four combinations (measured, weight), (measured, no weight), (flat, weight), (flat, no weight)
    ...
    sim_flat_electronic_noise_weight, sim_flat_electronic_noise_weight_electronic_comp = sim_with_weight(
            sim_flat_electronic_noise,
            calibration["no_measured_electronic_noise"],
            channel_ids=channel_ids)

    sim_measured_electronic_noise_no_weight, sim_measured_electronic_noise_no_weight_electronic_comp = sim_no_weight(
            sim_measured_electronic_noise,
            calibration["no_electronic_weight"],
            channel_ids=channel_ids)

    sim_flat_electronic_noise_no_weight, sim_flat_electronic_noise_no_weight_electronic_comp = sim_no_weight(
            sim_flat_electronic_noise,
            calibration["no_measured_electronic_noise_no_electronic_weight"],
            channel_ids=channel_ids)
    ...
    plt.style.use("retro")
    pdf_name = f"figures/electronic_noise/compare_electronic_noise_models_season{season}_station{station}.pdf"
    ...
    for i , channel_id in enumerate(channel_ids):
        ...
        ax_final_fit.plot(frequencies_data,
                          sim_flat_electronic_noise_weight[i],
                          label=f"flat 80 K noise\nwith fitted weight\n{calibration['no_measured_electronic_noise']['best_fit_template'][channel_id]}",
                          lw=2.
                          )
        ax_final_fit.plot(frequencies_data,
                          sim_measured_electronic_noise_no_weight[i],
                          label=f"measured noise\nno weight\n{calibration['no_electronic_weight']['best_fit_template'][channel_id]}",
                          lw=2.
                          )
        ax_final_fit.plot(frequencies_data,
                          sim_flat_electronic_noise_no_weight[i],
                          label=f"flat 80 K noise\nno weight\n{calibration['no_measured_electronic_noise_no_electronic_weight']['best_fit_template'][channel_id]}",
                          lw=2.
                          )
        ...
        ax_electronic.plot(frequencies_data,
                           sim_flat_electronic_noise_weight_electronic_comp[i],
                label="flat 80 K noise\nwith fitted weight")
        ax_electronic.plot(frequencies_data,
                           sim_measured_electronic_noise_no_weight_electronic_comp[i],
                label="measured noise\nno weight")
        ax_electronic.plot(frequencies_data,
                           sim_flat_electronic_noise_no_weight_electronic_comp[i],
                label="flat 80 K noise\nno weight")
