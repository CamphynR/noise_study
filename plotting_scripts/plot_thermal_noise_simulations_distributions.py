...
if __name__ == "__main__":
    ...
    sim_files = glob.glob(f"{sim_files}/events*.nur")
    print(sim_files)
...
#    hist_range = config["variable_function_kwargs"]["clean"]["hist_range"]
    ...
    freq_range = [200*units.MHz, 500*units.MHz]
...
#        ax.set_ylim(0., 0.007)
    ...
    for i , ax in enumerate(axs):
        ...
        sigma, cov = curve_fit(rayleigh, bin_centres, spec_amplitude_histograms_ice[channel_id, freq_idx,:], p0=sigma_guess)
        ax.stairs(spec_amplitude_histograms_ice[channel_id, freq_idx,:], edges=bin_edges)
