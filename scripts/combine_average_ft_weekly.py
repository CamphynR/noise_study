...
if __name__ == "__main__":
    ...
    for i, pickle in enumerate(args.pickles[1:]):
        ...
        prev_month = Time(header_prev["begin_time"], format="unix").datetime.month
        prev_week = Time(header_prev["begin_time"], format="unix").datetime.isocalendar()[1]
        week = Time(header["begin_time"], format="unix").datetime.isocalendar()[1]
        if prev_week != week:
            print("New week, saving \n ----------")
            ...
            pickle_file += f"_week{prev_week}_month_{prev_month}_combined.pickle"
