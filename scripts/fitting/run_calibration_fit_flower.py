...
def read_radiant_calibration(filepath):
    radiant_calibration = pd.read_csv(filepath)
    return radiant_calibration
...
def plot_spectra(spectrum_fitter, args, pdf, fit_results_dic, flower_responses, color="poster"):
    ...
    for idx, channel_id in enumerate(channel_ids):
        ...
        flower_sim_sum = np.zeros_like(frequencies)
        for i, sim in enumerate(args.sims):
            frequencies, frequency_spectrum, var_frequency_spectrum, header = read_freq_spec_file(sim)
            flower_sim_sum += frequency_spectrum[channel_id]
...
#                elec_temp = 80
            ...
            if noise_sources[i] == "electronic":
                labels[i] += "\n" + r"($T_{el}|" + f"{f0_factor} GHz$" +  f"= {elec_temp:.2f}" + r"$\pm$" + f"{sigma_elec_temp:.2f} K)"
                linewidth = 3.
            elif noise_sources[i] == "ice":
                pass
                linewidth = 3.
            ...
            frequency_spectrum *= flower_responses
        ...
        secax = ax.twinx()
        secax.plot(frequencies, flower_responses[channel_id], label = "system response")
#        secax.plot(frequencies, flower_sim_sum * flower_responses[channel_id], label = "normalization method")
        secax.legend()
        secax.set_ylabel("system response scale")
...
# Normalization way of getting flower gain
# --------------------------------------------
    flower_response_freq = np.fft.rfftfreq(1024, d=1./0.472)
#    season = 2023
#    radiant_calibration = read_radiant_calibration(f"absolute_amplitude_results/absolute_amplitude_calibration_season{season}_st{station_id}.csv")
#    radiant_gains = radiant_calibration["gain"][config["channels_to_include"]].to_numpy()
#    print(f"/user/rcamphyn/noise_study/absolute_amplitude_results/absolute_amplitude_calibration_season{season}_st{station_id}.csv")
#    print(radiant_calibration["gain"])
#
#    system_response_radiant = systemResponseTimeDomainIncorporator()
#    system_response_radiant.begin(response_path = "sim/library/deep_impulse_responses.json", det=0, normalize=True)
#    radiant_normalization = system_response_radiant.get_normalization(channel_id=0)
    
    # calibrated 22 is not in database
    if station_id == 22:
        station_id_db = 13
    else:
        station_id_db = station_id
    calibration_db_date = Time("2025-10-30", format="isot") 
    calibration_db = Detector(log_level=logging.WARNING,
                              always_query_entire_description=True,
                              select_stations=station_id_db,
    ...
    flower_responses = [
            np.abs(calibration_db.get_signal_chain_response(station_id_db, ch_id, trigger=True)(flower_response_freq)) for ch_id in config["channels_to_include"]]
...
#    impedance_mismatch_correction["VPol"] = interp1d(impedance_mismatch_correction_npz["frequencies"], impedance_mismatch_correction_npz["vpol"], bounds_error=False, fill_value=0.) 
#    
#    flower_responses_corrected = flower_responses * np.abs(impedance_mismatch_correction["VPol"](flower_response_freq))

    flower_responses = [f / np.max(f) for f in flower_responses_corrected]
#    system_response_flower = systemResponseTimeDomainIncorporator()
#    system_response_flower.begin(response_path = "sim/library/flower_impulse_responses.json", det=0, normalize=False)
#    flower_responses = [
#            np.max(radiant_gains[ch_id] * system_response_flower.get_response(ch_id)["gain"](flower_response_freq) / radiant_normalization) for ch_id in config["channels_to_include"]]
#    flower_responses = [
#            system_response_flower.get_response(ch_id)["gain"](flower_response_freq) for ch_id in config["channels_to_include"]]

    flower_responses = np.array(flower_responses)



# Conventional way of getting calibration
# ---------------------------------------
    bandpass_filt = channelBandPassFilter()
    
    bandpass = [0.01, 0.225]
    fit_range = [0.1, 0.2]
    fit_function_parameter_guesses = dict(gain=1000.,
                                          el_ampl=0.,
                                          el_cst=1.,
                                          f0=0.05)
    spectrum_fitter = spectrumFitter(args.data, args.sims, fit_range=fit_range,
                                     bandpass=bandpass, bandpass_type="gaussian_tapered",
                                     system_response=flower_responses,
                                     fit_function_parameter_guesses=fit_function_parameter_guesses)
    ...
    try:
        ...
        season = Time(spectrum_fitter.data_header["begin_time"], format="unix").strftime("%Y")

    print(season)


    plt.style.use("retro")
    savename = f"figures/absolute_ampl_calibration/spectra_fit_season{season}_st{station_id}_flower.pdf"
    ...
    with open(f"absolute_amplitude_fits/absolute_gains_fit_season{season}_s{station_id}_flower.pickle", "wb") as pickle_file:
...
#    fig.suptitle(f"Calibration results", size=62)
    ...
    plt.savefig(f"figures/POS_ICRC/spectra_fit_season{season}_st{station_id}_pos_flower.pdf", bbox_inches="tight") 
...
#-----------GAINS--------------------------------------------------------------------------------
    ...
    fig.suptitle(f"Station {station_id}")
    fig.tight_layout()
    fig.savefig(f"figures/absolute_ampl_calibration/gain_per_channel_season{season}_st{station_id}_flower") 
